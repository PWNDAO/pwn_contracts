...
import "../../src/loan-factory/lib/PWNSignatureChecker.sol";
...
abstract contract PWNSignatureCheckerTest is Test {
    ...
    function _sign(uint256 pk, bytes32 _digest) internal returns (bytes memory) {
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(pk, _digest);
        return abi.encodePacked(r, s, v);
    }

    function _signCompact(uint256 pk, bytes32 _digest) internal returns (bytes memory) {
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(pk, _digest);
...
contract PWNSignatureChecker_isValidSignatureNow_Test is PWNSignatureCheckerTest {
    ...
    function test_shouldFail_whenSignerIsEOA_whenSignatureHasWrongLength() external {
        ...
        vm.expectRevert("PWNSignatureChecker: Invalid signature length");
