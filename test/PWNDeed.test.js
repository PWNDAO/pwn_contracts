...
describe("PWNDeed contract", function() {
	...
	before(async function() {
		...
		deedEventIface = new ethers.utils.Interface([
			"event DeedCreated(address indexed assetAddress, uint8 category, uint256 id, uint256 amount, uint256 expiration, uint256 indexed did)",
			"event OfferMade(address assetAddress, uint8 category, uint256 amount, address indexed lender, uint256 toBePaid, uint256 indexed did, bytes32 offer)",
	...
	describe("Create", function() { // -> PWN is trusted source so we believe that it would not send invalid data
		...
		it("Should mint deed ERC1155 token", async function () {
			await deed.create(asset1.address, CATEGORY.ERC20, 1, 100, 54, borrower.address);
		...
		it("Should save deed data", async function () {
			...
			await deed.create(asset1.address, CATEGORY.ERC20, assetId, amount, expiration, borrower.address);
			const did = await deed.id();
		...
		it("Should return minted deed ID", async function() {
			const did = await deed.callStatic.create(asset1.address, CATEGORY.ERC20, 1, 100, 54, borrower.address);
		...
		it("Should emit NewDeed event", async function() {
			...
			const did = await deed.callStatic.create(asset1.address, CATEGORY.ERC20, assetId, amount, expiration, borrower.address);
			const tx = await deed.create(asset1.address, CATEGORY.ERC20, assetId, amount, expiration, borrower.address);
	...
	describe("Revoke", function() {
		...
		beforeEach(async function() {
			...
			await deed.create(asset1.address, CATEGORY.ERC20, 1, 100, expiration, borrower.address);
			did = await deed.id();
		});
	...
	describe("Make offer", function() { // -> PWN is trusted source so we believe that it would not send invalid data
		...
		it("Should fail when sender is not PWN contract", async function() {
			try {
				await deed.connect(addr1).makeOffer(asset2.address, CATEGORY.ERC20, 100, lender.address, did, 70);
		...
		it("Should fail when deed is not in new/open state", async function() {
			...
			try {
				await deed.makeOffer(asset2.address, CATEGORY.ERC20, 100, lender.address, did, 70);
		...
		it("Should save offer data", async function() {
			...
			await deed.makeOffer(asset2.address, CATEGORY.ERC20, amount, lender.address, did, toBePaid);
		...
		it("Should return offer hash as bytes", async function() {
			const offerHash = await deed.callStatic.makeOffer(asset2.address, CATEGORY.ERC20, 100, lender.address, did, 70);
		...
		it("Should increase global nonce", async function() {
			await deed.makeOffer(asset2.address, CATEGORY.ERC20, 100, lender.address, did, 70);
			await deed.makeOffer(asset2.address, CATEGORY.ERC20, 101, lender.address, did, 70);
		...
		it("Should emit NewOffer event", async function() {
			...
			const offerHash = await deed.callStatic.makeOffer(asset2.address, CATEGORY.ERC20, amount, lender.address, did, toBePaid);
			const tx = await deed.makeOffer(asset2.address, CATEGORY.ERC20, amount, lender.address, did, toBePaid);
			const response = await tx.wait();
	...
	describe("Revoke offer", function() { // -> PWN is trusted source so we believe that it would not send invalid data
		...
		beforeEach(async function() {
			...
			offerHash = await deed.callStatic.makeOffer(asset1.address, CATEGORY.ERC20, 100, lender.address, did, 101);
			await deed.makeOffer(asset2.address, CATEGORY.ERC20, 100, lender.address, did, 101);
		});
	...
	describe("Accept offer", function() { // -> PWN is trusted source so we believe that it would not send invalid data
		...
		beforeEach(async function() {
			...
			offerHash = await deed.callStatic.makeOffer(asset2.address, CATEGORY.ERC20, 100, lender.address, did, 101);
			await deed.makeOffer(asset2.address, CATEGORY.ERC20, 100, lender.address, did, 101);
		});
	...
	describe("Claim", function() {
		...
		beforeEach(async function() {
			await deed.create(asset1.address, CATEGORY.ERC20, 1, 100, timestampFromNow(duration), borrower.address);
			did = await deed.id();
	...
	describe("Before token transfer", function() {
		...
		beforeEach(async function() {
			...
			await deed.create(asset1.address, CATEGORY.ERC20, 1, 12, expiration, borrower.address);
			did = await deed.id();
		});
	...
	describe("View functions", function() {
		...
		beforeEach(async function() {
			...
			await deed.create(asset1.address, CATEGORY.ERC20, cAssetId, cAmount, expiration, borrower.address);
			did = await deed.id();

			offerHash = await deed.callStatic.makeOffer(asset2.address, CATEGORY.ERC20, lAmount, lender.address, did, lToBePaid);
			await deed.makeOffer(asset2.address, CATEGORY.ERC20, lAmount, lender.address, did, lToBePaid);
		});
